...
from mmdet3d.models.detectors.imgeonet import get_points
...
class OccupancyHead(nn.Module):
    def __init__(
        ...
        self.pre_proj = nn.Sequential(
            nn.Conv3d(in_channels, hidden_channels, 3, padding=1, bias=True),
            nn.LeakyReLU(),
        )
        self.net = OccupancyNet(hidden_channels, n_blocks, hidden_channels)
        self.post_proj = nn.Conv3d(hidden_channels, 1, 3, padding=1, bias=False)
        
        self.loss = build_loss(loss)

    def init_weights(self):
        normal_init(self.net, std=.01, bias=1)

    def forward(self, feat3d, valid, apply_sigmoid=True):
        # feat3d: (1, 256, num_x_voxels, num_y_voxels, num_z_voxels)
        # valid: (1, 1, num_x_voxels, num_y_voxels, num_z_voxels)
        # (1, 1, num_x_voxels, num_y_voxels, num_z_voxels)
        weight3d = self.post_proj(self.net(self.pre_proj(feat3d)))
        result = weight3d * valid.detach()
        return result.sigmoid() if apply_sigmoid else result

    def forward_test(self, feat3d, valid):
        with torch.no_grad():
            weight3d = self.forward(feat3d, valid)
        return weight3d

    def forward_train(self, feat3d, valid, target_occ):
        weight3d = self.forward(feat3d, valid, apply_sigmoid=False)

        # classes: 0=occupied, 1=not occupied
        loss = self.loss(weight3d.reshape(-1, 1), (~target_occ).long().reshape(-1))

        weight3d = weight3d.sigmoid()
        weight3d_binary = (weight3d > 0.5).detach()
        hit = (weight3d_binary[valid] == target_occ[valid]).sum()
        return weight3d, {'loss_occ': loss, 'acc_occ': hit / valid.sum()}
